...
The `agents/` package implements a **manager + 8 specialist subgraph** architecture built on **LangGraph's native `StateGraph`**.
...
| `_subagents.py` | `TOPIC_NAMES` list + `build_topic_subgraphs()` — compiles 8 topic subgraphs |
...
4. 8 topic subgraphs run in parallel, each:
...
5. compile_report node (runs after all 7 complete)
   │   → LLM call with all 8 topic results
...
class AnalysisState(TypedDict, total=False):
    ...
    quantitative_overview_result: dict | None
    # Final output
    final_report: str
```
...
| `TestTopicTools` | All 8 topics in `TOPIC_TOOLS`; missing cache key → error JSON |
| `TestSubagents` | 8 subgraphs built; each is a compiled StateGraph |
...
7. **Parallel fan-out** — LangGraph executes all 8 subgraphs concurrently; `compile_report` waits for all.
