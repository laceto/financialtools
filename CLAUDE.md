...
| File | Responsibility |
|---|---|
| `chains.py` | LangChain pipeline: reads Excel results → invokes `gpt-4.1-nano` → returns `StockRegimeAssessment`. Signature: `get_stock_evaluation_report(ticker, sector, year?, base_dir)`. Uses `PydanticOutputParser` directly (no `OutputFixingParser`). |
| `tools.py` | `make_tools(base_dir)` factory — returns one `@tool` (`get_stock_regime_report(ticker, sector, year?)`). `TOOLS = make_tools()` is the in-repo default. All tools return JSON strings, never raise. |
...
**LLM report (from Excel files, chains.py at repo root):**
```
get_stock_evaluation_report(ticker, sector, year?, base_dir)
...
**Agent (tools.py at repo root):**
```
# In-repo default:
TOOLS = make_tools()   # base_dir="financial_data"

# External consumer — path baked in at bootstrap, never exposed to the LLM:
tools = make_tools(base_dir="/path/to/data")

create_agent(model=llm, tools=tools, checkpointer=MemorySaver())
  get_stock_regime_report(ticker, sector, year?) → get_stock_evaluation_report(base_dir) → JSON (model_dump())
```
...
- `SectorNotFoundError` — raised by `chains.get_stock_evaluation_report` (indirectly, if sector is not found in benchmark files). Inherits `ValueError` so `except ValueError` blocks still work.
...
| `SectorNotFoundError` | Sector missing from benchmark Excel file (`financial_data/metrics_by_sectors.xlsx`) |
